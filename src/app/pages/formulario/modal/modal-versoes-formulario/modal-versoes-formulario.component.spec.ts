...
import { ModalVersoesFormularioComponent } from './modal-versoes-formulario.component';
import { NgbModal, NgbModalRef } from '@ng-bootstrap/ng-bootstrap';
import { TabelaSortableHeader } from "../../../../shared/tables/table-sortable";
import { QueryList } from '@angular/core';
import {ModalConstruirFormularioComponent} from "../modal-construir-formulario/modal-construir-formulario.component";
import {PrincipalModule} from "../../../principal/principal.module";
import {AppModule} from "../../../../app.module";
import {HttpClientTestingModule} from "@angular/common/http/testing";
import {AlertUtils} from "../../../../../utils/alerts.util";

describe('ModalVersoesFormularioComponent', () => {

  let component: ModalVersoesFormularioComponent;
  let fixture: ComponentFixture<ModalVersoesFormularioComponent>;
  ...
  beforeEach(async () => {
    ...
    fixture = TestBed.createComponent(ModalVersoesFormularioComponent);
  ...
  it('should open modal and set data correctly',(done) => {
    const formularioList = [{ id: 1 }];
    const totalElements = 1;
    const descricao = 'Test Modal';

    component.open(formularioList, totalElements, descricao).then((result) => {
  ...
  it('should call tableLazyLoading correctly', () => {
    const formularioList = [{ id: 1, versao: 2 }];
    const totalElements = 1;

    component.formularioList = formularioList;
    component.totalElements = totalElements;
    component.tableLazyLoading();

    expect(component.page.content).toEqual(formularioList);
  ...
  it('should emit construirFormulario event', () => {
    spyOn(component.construirFormulario, 'emit');
    const item = { id: 1 };

    component.construirformulario(item);

    expect(component.construirFormulario.emit).toHaveBeenCalledWith(item);
  });
  ...
  it('should return correct modification data', () => {
    ...
    expect(component.getDataModificacao(item)).toBe('2022-01-01');

    const itemWithoutAlteracao = { dataAlteracao: null, dataRegistro: '2021-01-01' };
    expect(component.getDataModificacao(itemWithoutAlteracao)).toBe('2021-01-01');
  });
  ...
  it('should return correct modification user', () => {
    ...
    expect(component.getUsuarioModificacao(item)).toBe('user2');

    const itemWithoutAlteracao = { usuarioAlteracao: null, usuarioRegistro: 'user1', dataAlteracao: null };
    expect(component.getUsuarioModificacao(itemWithoutAlteracao)).toBe('user1');
  });
});
