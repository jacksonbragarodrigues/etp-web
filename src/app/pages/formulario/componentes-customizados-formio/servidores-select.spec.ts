...
import ServidorSelect, { simplePlugin } from './servidores-select';
...
describe('ServidorSelect', () => {
  let component: ServidorSelect;
  ...
  beforeEach(() => {
    ...
    component = new ServidorSelect(mockComponent, options, data);
  });
  ...
  it('deve inicializar o componente corretamente', () => {
    const expectedUrl =
      getEnvironment().apiFormulario + '/sarhclient/listaservidores';
  ...
  it('should return correct builder info properties', () => {
    ...
    spyOn(ServidorSelect, 'schema').and.returnValue({});

    const builderInfo = ServidorSelect.builderInfo;

    // Verifica se o título está correto
    expect(builderInfo.title).toBe('Servidores');
    ...
    expect(ServidorSelect.schema).toHaveBeenCalledWith({});
  });
});
